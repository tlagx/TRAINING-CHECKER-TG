...
const badgeDataWithCriteria = badgeData.map((badge) => {
    ...
    switch (badge.translationKey) {
        ...
        case "exteam":
            ...
            break;
        case "bot":
            criteriaFunction = (playerData, additionalInfo) => playerData.moder === 997 || playerData.verify === 6;
            break;
        case "chronos":
            criteriaFunction = (playerData, additionalInfo) => playerData.id === 644835;
            break;
        default:
            criteriaFunction = (playerData, additionalInfo) => false;
    }
    return Object.assign(Object.assign({}, badge), { criteria: criteriaFunction });
});
export const playerInfoHandler = async (ctx, updateMessage = false) => {
    var _a;
    ...
    try {
        ...
        if (badgeTranslations && badgeTranslations.items) {
            const earnedBadges = badgeDataWithCriteria.filter((badge) => badge.criteria && badge.criteria(playerData, additionalInfo));
...
export const handleWarnsButton = async (ctx, nickname) => {
