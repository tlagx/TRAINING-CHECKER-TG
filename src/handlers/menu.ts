...
const userStates: Record<string, { waitingForNickname: boolean; previousMessageId?: number }> = {}
...
export const mainMenuHandler = async (ctx: any) => {
...
export const handleMenuSelection = async (ctx: any, action: string) => {
  const userId = ctx.from?.id.toString() || 'default'
  const userLang = await getUserLanguage(userId)
...
export const handleNicknameInput = async (ctx: any) => {
  const userId = ctx.from?.id.toString() || 'default'
  ...
  try {
    ...
    const badgeDataWithCriteria = badgeData.map((badge: any) => {
      ...
      switch (badge.translationKey) {
        ...
        case "exteam":
          ...
          break;
        case "bot":
          criteriaFunction = (playerData: any, additionalInfo: any) => playerData.verify === 6;
          break;
        case "chronos":
          criteriaFunction = (playerData: any, additionalInfo: any) => playerData.verify === 7;
          break;
        default:
    ...
    const earnedBadges = badgeDataWithCriteria.filter(badge => 
      badge.criteriaFunction(playerData, additionalInfo)
    );
    ...
    const errorMessage = t(userLang, 'common', 'player_info_error', { error: (error as Error).message });
    const backKeyboard = Markup.inlineKeyboard([
      [Markup.button.callback(t(userLang, 'common', 'menu_back'), 'menu_main')]
    ]);
    try {
